...
public class DbStructImpl implements DbStruct {
	...
	private void checkModify(DbProperties dbProperties, SimpleDalContext ctx, TableMeta tableMeta, TableObjectStruct tableStruct)
		...
		//检查表信息
		if (!com.gitlab.summercattle.commons.utils.auxiliary.StringUtils.equals(tableMeta.getComment(), tableStruct.getComment())) {
		...
		for (IndexMeta indexMeta : indexMetas) {
			...
			String idxName = DataConstants.INDEX_KEY_PREFIX + com.gitlab.summercattle.commons.utils.auxiliary.StringUtils.getHashName("Table'"
	...
	private void processTableColumn(SimpleDalContext ctx, String tableName, TableFieldStruct fieldStruct, DataType dataType, int length, int scale,
		...
		if (!com.gitlab.summercattle.commons.utils.auxiliary.StringUtils.equals(fieldStruct.getDefaultValue(), defaultValue)) {
			if (StringUtils.isNotBlank(defaultValue)) {
				if (dataType == DataType.Number) {
		...
		if (!com.gitlab.summercattle.commons.utils.auxiliary.StringUtils.equals(fieldStruct.getComment(), comment)) {
	...
	private void createTablePrimaryKey(DbProperties dbProperties, SimpleDalContext ctx, TableMeta tableMeta) throws CommonException {
		String pkName = DataConstants.PRIMARY_KEY_PREFIX + com.gitlab.summercattle.commons.utils.auxiliary.StringUtils
